...
if ( ! function_exists( 'aihr_check_aihrus_framework' ) ) {
	function aihr_check_aihrus_framework( $file = null, $name = null, $aihr_min = '1.0.0' ) {
		if ( is_null( $file ) ) {
			aihr_notice_error( __( '`aihr_check_aihrus_framework` requires $file argument', 'wootheme-testimonials-to-testimonials' ) );
...
if ( ! function_exists( 'aihr_notice_aihrus_framework' ) ) {
	function aihr_notice_aihrus_framework() {
		...
		if ( defined( 'AIHR_BASE' ) ) {
			...
			$note = sprintf( esc_html__( 'Plugin "%1$s" is causing the out of date issue.', 'wootheme-testimonials-to-testimonials' ), $plugin_name );
		}

		$aihr_version = defined( 'AIHR_VERSION' ) ? AIHR_VERSION : '0.0.0';

		$text = sprintf( esc_html__( 'Plugin "%1$s" has been deactivated as it requires Aihrus Framework %2$s or newer. You\'re running Aihrus Framework %4$s. Once corrected, "%1$s" can be activated. %5$s %3$s', 'wootheme-testimonials-to-testimonials' ), $name, AIHR_VERSION_MIN, $help_link, $aihr_version, $note );
...
if ( ! function_exists( 'aihr_check_php' ) ) {
	function aihr_check_php( $file = null, $name = null, $php_min = '5.3.0' ) {
		if ( is_null( $file ) ) {
			aihr_notice_error( __( '`aihr_check_php` requires $file argument', 'wootheme-testimonials-to-testimonials' ) );
...
if ( ! function_exists( 'aihr_notice_php' ) ) {
	function aihr_notice_php() {
		...
		$text = sprintf( __( 'Plugin "%1$s" has been deactivated as it requires PHP %2$s or newer. You\'re running PHP %4$s. Once corrected, "%1$s" can be activated. <a href="%3$s">More information</a>.', 'wootheme-testimonials-to-testimonials' ), $name, AIHR_PHP_VERSION_MIN, $help_url, PHP_VERSION );
...
if ( ! function_exists( 'aihr_check_wp' ) ) {
	function aihr_check_wp( $file = null, $name = null, $wp_min = '3.6.0' ) {
		if ( is_null( $file ) ) {
			aihr_notice_error( __( '`aihr_check_wp` requires $file argument', 'wootheme-testimonials-to-testimonials' ) );
...
if ( ! function_exists( 'aihr_notice_wp' ) ) {
	function aihr_notice_wp() {
		...
		$text = sprintf( __( 'Plugin "%1$s" has been deactivated as it requires WordPress %2$s or newer. You\'re running WordPress %4$s. Once corrected, "%1$s" can be activated. <a href="%3$s">Update WordPress</a>.', 'wootheme-testimonials-to-testimonials' ), $name, AIHR_WP_VERSION_MIN, $help_url, $wp_version );
...
if ( ! function_exists( 'aihr_notice_license' ) ) {
	function aihr_notice_license( $post_type, $settings_id, $free_name, $purchase_url, $item_name ) {
		...
		$text = __( '<a href="%1$s">%2$s &gt; Settings</a>, <em>Premium</em> tab, <em>License Key</em> entry', 'wootheme-testimonials-to-testimonials' );

		$settings_link = sprintf( $text, $link, $free_name );

		$link = esc_url( 'https://aihrus.zendesk.com/entries/28745227' );
		$text = __( '<a href="%s">Where\'s my license key?</a>', 'wootheme-testimonials-to-testimonials' );

		$faq_link = sprintf( $text, $link );

		$link = esc_url( $purchase_url );
		$text = __( '<a href="%1$s">%2$s</a>', 'wootheme-testimonials-to-testimonials' );

		$buy_link = sprintf( $text, $link, $item_name );

		$text = sprintf( __( 'Plugin "%1$s" requires license activation before updating will work. Please activate the license key through %2$s. No license key? See "%3$s" or purchase "%4$s".', 'wootheme-testimonials-to-testimonials' ), $item_name, $settings_link, $faq_link, $buy_link );
