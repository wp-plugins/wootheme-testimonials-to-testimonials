...
abstract class Aihrus_Settings {
	...
	public static function sections() {
		static::$sections['reset'] = esc_html__( 'Reset', 'wootheme-testimonials-to-testimonials' );
		static::$sections['about'] = esc_html__( 'About', 'wootheme-testimonials-to-testimonials' );
	}
	...
	public static function settings() {
		...
		if ( ! empty( $options ) ) {
			...
			static::$settings['export'] = array(
				'section' => 'reset',
				'title' => esc_html__( 'Export Settings', 'wootheme-testimonials-to-testimonials' ),
				'type' => 'readonly',
				'desc' => esc_html__( 'These are your current settings in a serialized format. Copy the contents to make a backup of your settings.', 'wootheme-testimonials-to-testimonials' ),
		...
		static::$settings['import'] = array(
			'section' => 'reset',
			'title' => esc_html__( 'Import Settings', 'wootheme-testimonials-to-testimonials' ),
			'type' => 'textarea',
			'desc' => esc_html__( 'Paste new serialized settings here to overwrite your current configuration.', 'wootheme-testimonials-to-testimonials' ),
			'widget' => 0,
		);

		$desc = esc_html__( 'Delete all %s data and options from database on plugin deletion', 'wootheme-testimonials-to-testimonials' );

		static::$settings['delete_data'] = array(
			'section' => 'reset',
			'title' => esc_html__( 'Remove Plugin Data on Deletion?', 'wootheme-testimonials-to-testimonials' ),
			'type' => 'checkbox',
			'class' => 'warning',
		...
		static::$settings['reset_defaults'] = array(
			'section' => 'reset',
			'title' => esc_html__( 'Reset to Defaults?', 'wootheme-testimonials-to-testimonials' ),
			'type' => 'checkbox',
			'class' => 'warning',
			'desc' => esc_html__( 'Check this box to reset options to their defaults', 'wootheme-testimonials-to-testimonials' ),
		);
	}
...
';
	...
	public static function display_page( $disable_donate = false ) {
		...
		echo '
			<p class="submit"><input name="Submit" type="submit" class="button-primary" value="' . esc_html__( 'Save Changes', 'wootheme-testimonials-to-testimonials' ) . '" /></p>
			</form>
		</div>
		';
		...
		if ( ! $disable_donate ) {
			echo '<p>' .
				sprintf(
				__( 'If you like this plugin, please <a href="%1$s" title="Donate for Good Karma"><img src="%2$s" border="0" alt="Donate for Good Karma" /></a> or <a href="%3$s" title="purchase premium WordPress plugins from Aihrus ">purchase the Premium version</a> to help fund further development and <a href="%4$s" title="Support forums">support</a>.', 'wootheme-testimonials-to-testimonials' ),
		...
		echo '<p class="copyright">' .
			sprintf(
			__( 'Copyright &copy;%1$s <a href="%2$s">Aihrus</a>.', 'wootheme-testimonials-to-testimonials' ),
			date( 'Y' ),
	...
	public static function validators( $validate, $id, &$input, $default, &$errors ) {
		switch ( $validate ) {
			...
			case 'required':
				if ( empty( $input[ $id ] ) )
					$errors[ $id ] = esc_html__( 'Required', 'wootheme-testimonials-to-testimonials' );
				break;
